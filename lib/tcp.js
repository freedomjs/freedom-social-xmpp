...
freedomTCP.prototype._writeNative = function(req, buf) {
  var promise = socket.write(this.fd, data);
...
freedomTCP.prototype.connect = function(cb, address, port) {
  ...
  console.warn('freedomTCP asked to connect on ' + port);
  var promise = socket.connect(this.fd, address, port);
  promise.done(cb.oncomplete);
};
