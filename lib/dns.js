...
var ipV4RegExp = new RegExp("^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$");
...
function getaddrinfo(domain, family) {
  ...
  queryDNS('8.8.8.8', msg, function(resp) {
    ...
    if (!('response' in obj)) { return ret.oncomplete(null); }
    var addresses = [];
...
exports.lookup = function(hostname, family, callback) {
  ...
  // TODO: what to return?
  // TODO: why does the old code set: callback.immediately = true;
  // TODO: remove any functions which are no longer needed
...
/*
  ...
  function onanswer(addresses) {
    if (addresses && addresses.length > 0) {
      ...
      callback(errnoException('ENOTFOUND', 'getaddrinfo'));
    }
  }
