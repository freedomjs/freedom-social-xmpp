/**
 * Node DNS partial implementation bound to freedom['core.socket'] calls.
 *
...
function queryDNS(server, msg, callback) {
  var promise = socket.create('udp', {});
  promise.then(function(_socketInfo) {
    var promise = socket.connect(_socketInfo.socketId, server, 53);
    promise.then(function(result) {
      if (result != 0) {
        console.log("Could not connect to server!");
        callback(null);
        socket.destroy(_socketInfo.socketId);
      }
      socket.on("onData", function(readInfo) {
        ...
        callback(readInfo);
        socket.disconnect(_socketInfo.socketId);
        socket.destroy(_socketInfo.socketId);
      });
      var write = socket.write(_socketInfo.socketId, msg);
      write.then(function(sr) {
        if (sr.bytesWritten != msg.byteLength) {
          console.log("Write was unsucessful.");
          callback(null);
          var disconnect = socket.disconnect(_socketInfo.socketId);
          disconnect.then(socket.destroy.bind({},_socketInfo.socketId));
        }
      });
    });
  });
...
exports.lookup = function(hostname, family, callback) {
  ...
  var wrap = getaddrinfo(hostname, family);
  if (!wrap) {
